...
<div class="toolbar">
    <form class="filter-row" method="GET">
        ...
        <label class="field"><span>{{ __('Status') }}</span><select name="status"><option value="">{{ __('All statuses') }}</option>@foreach(\App\Models\StudentReferral::STATUSES as $status)<option value="{{ $status }}" @selected(request('status') === $status)>{{ __(ucfirst($status)) }}</option>@endforeach</select></label>
...
<div class="card table-card"><div class="table-wrap"><table class="data-table">
    ...
    <tbody>@forelse($referrals as $referral)<tr>
        <td><strong>{{ $referral->student_name }}</strong><small class="link-destination" dir="ltr">{{ $referral->mobile }}</small><small class="link-destination">{{ $referral->reference_code }}</small></td>
        <td>{{ $referral->partner->name }}</td>
        <td>{{ $referral->nationality }}</td>
        ...
        <td><form method="POST" action="{{ route('crm.student-referrals.update', $referral) }}">@csrf @method('PATCH')<select class="status-select" name="status" onchange="this.form.submit()">@foreach(\App\Models\StudentReferral::STATUSES as $status)<option value="{{ $status }}" @selected($referral->status === $status)>{{ __(ucfirst($status)) }}</option>@endforeach</select></form></td>
...
</table></div>@if($referrals->hasPages())<div class="pagination">{{ $referrals->links() }}</div>@endif</div>
@endsection

@push('head')<style>.referral-stats{grid-template-columns:repeat(4,1fr)}.filter-row{flex:1}.filter-row .field:first-child{min-width:230px}.status-select{min-width:120px;padding:8px}.muted-text{color:#87938c;font-size:12px}@media(max-width:900px){.referral-stats{grid-template-columns:1fr 1fr}}@media(max-width:600px){.referral-stats{grid-template-columns:1fr}.filter-row .field{width:100%;min-width:0!important}}</style>@endpush
